...
阶段三真实压测进一步完成 8 请求突发 + 6 请求持续供货：14/14 成功、14 张均经 Pillow 验为 2048×2048，突发时生成 worker 自动扩到 5、空闲后回落到 1；后端/LiteLLM 峰值分别为 285.883/1160.184 MiB，均无重启或 OOM。默认三路持续供货的短窗实测约 113 张/小时；此前约 700 张/小时只是理论设计值，当前部署不能据此对外承诺。完整方法、GPU 显存和瓶颈见 [阶段三弹性压测报告](docs/elastic-load-validation.md)。压测结束后云端已恢复 `min=1/max=3`。
